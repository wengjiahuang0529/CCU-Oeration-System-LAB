 ...
 * @file     sdcard.c
 * @version  V1.00
 * @brief    NUC100 series SD Card source file
 *
 * @note
...
uint32_t MMC_Command_Exec(uint8_t nCmd, uint32_t nArg, uint8_t *pchar, uint32_t *response)
{
    ...
    PE7 = 0;//SPI_SET_SS_LOW(g_pSPI);// CS = 0
...
//      while((SingleWrite(0xFF)&0xFF) == 0x00);
    ...
    PE7 = 1;//SPI_SET_SS_HIGH(g_pSPI);// CS = 1
...
/*@}*/ /* end of group NUC100_SDCARD_Driver */
